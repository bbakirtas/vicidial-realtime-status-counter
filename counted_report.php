...
$select_list .= "<TR><TD align=right>";
$select_list .= _QXZ("Count Status").":  </TD><TD align=left><SELECT SIZE=1 NAME=GOSTATUS ID=GOSTATUS>";

$stmt="select * from vicidial_statuses where status !='NEW';";
$rslt=mysql_to_mysqli($stmt, $link);
if ($DB) {$OUToutput .= "$stmt\n";}
$count_status_select = mysqli_num_rows($rslt);
if ($count_status_select > 0)
	{
	while ($row=mysqli_fetch_row($rslt))
	{
	$select_list .= "<option value='$row[0]'";
	$select_list .= ">".$row[1]."</option>";
}
	}
...
var GOSTATUS = '<?php echo $GOSTATUS; ?>';
...
function update_variables(task_option,task_choice,force_reload)
	...
	if (task_option == 'form_submit')
		...
		var GOSTATUSFORM = document.getElementById('GOSTATUS');
		GOSTATUS = GOSTATUSFORM[GOSTATUSFORM.selectedIndex].value;
