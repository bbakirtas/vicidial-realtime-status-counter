...
if ($report_display_type=='TEXT')
	...
	$Countokeycall =			"  | "._QXZ("$GOSTATUS COUNT",12)." | ";
	if ($RS_DIDdesc > 0)
		{
		$HDigcall =			"------+-----------------------------+-----------------";
...
if ($realtime_block_user_info > 0)
	{
 	...
 	$Bline  = "$HTbegin$HTusergroup$HTsessionid$HDlisten$HTbarge$HTwhisper$HTstatus$HTpause$HTcustphone$HTcustinfo$HTserver_ip$HTcall_server_ip$HTtime$HTcampaign$HTcalls$HTastcall$HTigcall$Countokeycall</tr>\n";
	}
else
	{
 	...
 	$Bline  = "$HTbegin$HTstation$HTphone$HTuser$HTusergroup$HTsessionid$HTlisten$HTbarge$HTwhisper$HTstatus$HTpause$HTcustphone$HTcustinfo$HTserver_ip$HTcall_server_ip$HTtime$HTcampaign$HTcalls$HTastcall$HTigcall$Countokeycall</tr>\n";
	}
$Aecho .= "$Aline";
$Aecho .= "$Bline";
$Aecho .= "$Aline";
...
#				{$G='<SPAN class="orange"><B>'; $EG='</B></SPAN>';}
		...
		if ($report_display_type=='TEXT')
			...
			$stmt="SELECT count(*) from vicidial_list where modify_date >='$datemodify 00:00:00' and user='$Auser[$i]' and status='$GOSTATUS'";
			$rslt=mysql_to_mysqli($stmt, $link);
			$ast_to_count_ok = mysqli_num_rows($rslt);
			if ($ast_to_count_ok > 0)
					{
					$row=mysqli_fetch_row($rslt);
					$CNTOKYCLL =  $row[0];
					}
			...
			if ($filtered_ingroup==1) 
				...
				if ($realtime_block_user_info > 0)
					{
					$Aecho .= "|$UGD $G$sessionid$EG$L$R$Aring_note[$i]| $G"._QXZ("$status",6)."$EG $CM $pausecode|$CP$CI$SVD$G$call_time_MS$EG | $G$campaign_id$EG | $G$calls_today$EG |$ASTCT$INGRP |$G$CNTOKYCLL$EG |\n";
					}
				if ($realtime_block_user_info < 1)
					{
					$Aecho .= "| $G$extension$EG$Aring_note[$i]|$phoneD<a href=\"./user_status.php?user=$Luser\" target=\"_blank\">$G$user$EG</a> <a href=\"javascript:ingroup_info('$Luser','$j');\">+</a> |$UGD $G$sessionid$EG$L$R | $G"._QXZ("$status",6)."$EG $CM $pausecode|$CP$CI$SVD$G$call_time_MS$EG | $G$campaign_id$EG | $G$calls_today$EG |$ASTCT$INGRP |$G$CNTOKYCLL$EG |\n";
					}
				}
